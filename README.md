...
當系統實體記憶體使用率超過門檻時，自動清空待命快取（standby）並以「終止整棵子進程樹」的方式終止最吃記憶體的程式（預設鎖定 AI / llama 類工作負載），記憶體壓力解除後停止介入。

編譯出來的單一可執行檔約 430 KB，無外部執行期相依。

## 功能
- **滯回式壓力偵測**：使用率 ≥ 觸發門檻進入壓力；要低於「恢復門檻」才解除，避免門檻附近反覆震盪。
- **Standby 回收**：壓力期間定期呼叫 `SetSystemFileCacheSize` 清待命快取（每 2 秒至多一次）。
...
建議以**管理員身分**執行，否則終止其他使用者／高權限進程、清空快取可能失敗。
